...
class BarcodeEAN
{
    ...
    public static function getDigit($code, $type)
    {
        ...
        if ($type == 'ean8') {
            // process left part
            for ($i = 0; $i < 4; $i++) {
                $result .= self::$encoding[(int)$code[$i]][0];
            }
            ...
            // process right part
            for ($i = 4; $i < 8; $i++) {
                $result .= self::$encoding[(int)$code[$i]][2];
            }

        } else { // ean13
            // extract first digit and get sequence
            $seq = self::$first[(int)$code[0]];

            // process left part
            for ($i = 1; $i < 7; $i++) {
                $result .= self::$encoding[(int)$code[$i]][(int)$seq[$i - 1]];
            }
...
class BarcodeMSI
{
    ...
    public static function getDigit($code)
    {
        ...
        $len = strlen($code);
        for ($i = 0; $i < $len; $i++) {
            $result .= self::$encoding[(int)$code[$i]];
        }
...
class Barcode128
{
    ...
    public static function getDigit($code)
    {
        ...
        while ($i < $len) {
            ...
            if ($tableCActivated) {
                $value = (int)substr($code, $i, 2); // Add two characters (numeric)
                $i += 2;
            } else {
